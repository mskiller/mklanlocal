...
from media_indexer_backend.services.asset_service import _asset_summary, get_asset_or_404
...
def _ingest_bytes_to_directory(
    session: Session,
    *,
...
def ingest_generated_file_to_inbox(
    ...
    *,
    ...
    source = _upload_source(session)
    default_target = _default_target_source(session)
    media_type = detect_media_type(Path(filename), guess_mime_type(Path(filename)))
    if media_type == MediaType.UNKNOWN:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"{filename} is not a supported media file.")

    target_directory, normalized_folder = resolve_writable_directory_path(source.root_path, folder)
    target_directory.mkdir(parents=True, exist_ok=True)
    return _ingest_bytes_to_directory(
        session,
        target_directory=target_directory,
        normalized_folder=normalized_folder,
        filename=filename,
...
def approve_inbox_item(
    ...
    *,
    ...
    target_source = get_source_or_404(session, resolved_target_source_id, current_user=current_user)
...
def inbox_compare_payload(session: Session, item_id: UUID, *, current_user: User):
    item = get_inbox_item_or_404(session, item_id)
    nearest_asset = None
    if item.nearest_asset_id:
        try:
            nearest_asset = get_asset_or_404(session, item.nearest_asset_id, current_user=current_user)
        except HTTPException:
            nearest_asset = None
    return {
